...
class IABot {
    constructor() {
        ...
        // Ma'lumotlar bazasi - 4 ta yo'nalish bo'yicha
        this.knowledgeBase = {
            robototexnika: {
        ...
        topicButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (this.currentTopic !== 'all') {
                    this.addBotMessage(`✅ Endi faqat **${this.getTopicName(this.currentTopic)}** bo'yicha javob beraman. Savolingizni yozing!`);
                } else {
                    this.addBotMessage("✅ Endi barcha yo'nalishlar bo'yicha javob beraman. Qaysi mavzuda savolingiz bor?");
                }
            });
        });
    }
    ...
    handleUserMessage() {
        ...
        }, 1000 + Math.random() * 1000); // 1-2 soniya kutish
    }
    ...
    generateResponse(message) {
        ...
        // Salomlashish
        if (/salom|assalom|hello|hi|hey/.test(lowerMessage)) {
            return this.getGreeting();
        }
        
        // Rahmat
        if (/rahmat|thank|tashakkur/.test(lowerMessage)) {
            return "😊 Arzimaydi! Boshqa savolingiz bo'lsa, bemalol so'rang. Yordam berishdan xursandman!";
        }
        
        // Xayr
        if (/xayr|bye|goodbye|alvido/.test(lowerMessage)) {
            return "👋 Xayr! Qayta uchrashgunga qadar. O'qinganingizda omad tilayman!";
        }
        ...
        for (const [topic, data] of Object.entries(this.knowledgeBase)) {
            const score = this.calculateScore(lowerMessage, data.keywords);
            if (score > maxScore) {
                maxScore = score;
                bestTopic = topic;
            }
        ...
        if (bestTopic && maxScore > 0) {
            const topicData = this.knowledgeBase[bestTopic];
            return this.findBestResponse(lowerMessage, topicData);
        }
    ...
    calculateScore(message, keywords) {
        ...
        keywords.forEach(keyword => {
            ...
            // To'liq moslik uchun bonus
            if (message === keyword) score += 3;
        });
        return score;
    }
    
    findBestResponse(message, topicData) {
        const responses = topicData.responses;
        
        // Maxsus kalit so'zlarni tekshirish
        if (message.includes('arduino') || message.includes('sensor') || message.includes('dvigatel') || message.includes('motor')) {
            if (message.includes('arduino')) return responses.arduino || responses.default;
            if (message.includes('sensor')) return responses.sensors || responses.default;
            if (message.includes('dvigatel') || message.includes('motor')) return responses.motors || responses.default;
        }
        
        if (message.includes('html') || message.includes('css') || message.includes('javascript') || message.includes('js') || message.includes('react')) {
            if (message.includes('html')) return responses.html || responses.default;
            if (message.includes('css')) return responses.css || responses.default;
            if (message.includes('javascript') || message.includes('js')) return responses.javascript || responses.default;
            if (message.includes('react')) return responses.react || responses.default;
        }
        
        if (message.includes('premiere') || message.includes('davinci') || message.includes('after') || message.includes('capcut') || message.includes('color') || message.includes('effekt')) {
            if (message.includes('premiere')) return responses.premiere || responses.default;
            if (message.includes('davinci')) return responses.davinci || responses.default;
            if (message.includes('after')) return responses.effects || responses.default;
            if (message.includes('capcut') || message.includes('mobile')) return responses.mobile || responses.default;
            if (message.includes('color')) return responses.color || responses.default;
        }
        
        if (message.includes('blender') || message.includes('3d') || message.includes('model') || message.includes('render') || message.includes('unity') || message.includes('unreal')) {
            if (message.includes('blender')) return responses.blender || responses.default;
            if (message.includes('model')) return responses.modeling || responses.default;
            if (message.includes('textur')) return responses.texturing || responses.default;
            if (message.includes('render')) return responses.rendering || responses.default;
            if (message.includes('animat')) return responses.animation || responses.default;
            if (message.includes('unity') || message.includes('unreal') || message.includes('game')) return responses.game || responses.default;
        }
        
        // Umumiy mavzu savollari
        if (message.includes('nima') || message.includes('what') || message.includes('?')) {
            return responses.greeting || responses.default;
        }
        
        return responses.default || "Bu haqida batafsil ma'lumot uchun aniqroq savol bering.";
    }
    
    getGreeting() {
        const greetings = [
            "👋 Assalomu Alaykum! Men INNO HUB . Sizga quyidagi yo'nalishlarda yordam beraman:\n\n🤖 Robototexnika\n💻 Web Dasturlash\n🎬 Mobilografiya\n🎨 3D Modellashtirish\n\nQaysi mavzu haqida gaplashmoqchisiz?",
            "🚀 Assalomu alaykum! Texnologiya dunyosiga xush kelibsiz!\n\nMening ixtisosligim:\n- Arduino va robototexnika\n- Web dasturlash (Frontend/Backend)\n- Video montaj va mobilografiya\n- 3D modellashtirish\n\nSavolingizni ayting!"
        ];
